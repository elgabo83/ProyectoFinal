...
def ropa_masculina(request):
    return render(request, 'coder_app/ropa-masculina.html')
...
def lanzamientos(request):
    if request.method == 'POST' and 'submit_lanzamientos' in request.POST:
        form_lanzamientos = ImageUploadForm(request.POST, request.FILES)
        if form_lanzamientos.is_valid():
            image = form_lanzamientos.save(commit=False)
            image.uploaded_by = request.user
            image.category = 'lanzamientos'
            image.save()
        ...
        form_lanzamientos = ImageUploadForm()
    
    images_lanzamientos = Image.objects.filter(category='lanzamientos')
    return render(request, 'coder_app/lanzamientos.html', {'images': images_lanzamientos, 'form_lanzamientos': form_lanzamientos})
pass
